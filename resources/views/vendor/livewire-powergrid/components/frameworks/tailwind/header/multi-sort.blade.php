        ...
        <span class="flex gap-2 select-none">
            @foreach($sortArray as $field => $sort)
                ...
                <span
                    ...
                    class="group gap-2 cursor-pointer border border-pg-primary-200 inline-flex rounded-full items-center py-0.5 pl-2.5 pr-1 text-sm font-medium bg-pg-primary-100 text-pg-primary-700">
                    {{ $label }}
                    @if($sort == 'desc')
